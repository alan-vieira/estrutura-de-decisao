# 19. Faça um Programa que leia um número inteiro menor que 1000 e
# imprima a quantidade de centenas, dezenas e unidades do mesmo.
#
# Observando os termos no plural a colocação do "e", da vírgula
# entre outros. Exemplo:
#
# 326 = 3 centenas, 2 dezenas e 6 unidades
# 12 = 1 dezena e 2 unidades Testar com: 326, 300, 100, 320,
# 310,305, 301, 101, 311, 111, 25, 20, 10, 21, 11, 1, 7 e 16

numero = int(input("Digite um numéro inteiro menor que 1000: "))


un = "unidades"
de = "dezenas"
ce = "centenas"

if(numero < 1000):
    unidade = numero // 1 % 10
    dezena = numero // 10 % 10
    centena = numero // 100 % 10

    if(unidade == 1):
        un = "unidade"
    if(dezena == 1):
        de = "dezena"
    if(centena == 1):
        ce = "centena"

    if(centena == 0 and dezena != 0):
        print(numero, "=",dezena, de, "e", unidade, un)
    elif(centena == 0 and dezena == 0):
        print(numero, "=",unidade, un)
    else:
        print(numero,"=",centena,ce,",",dezena,de,"e",unidade,un)
else:
    print("O numéro é maior que 1000")